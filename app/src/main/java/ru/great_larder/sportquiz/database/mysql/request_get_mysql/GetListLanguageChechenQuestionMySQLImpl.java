...
public class GetListLanguageChechenQuestionMySQLImpl implements GetListQuestionMySQL {
	@Override
	public List<Question> getListQuestion(User user, Context context) {
		...
		try {
			connectMySQL.resultSetMySQL = connectMySQL.statementMySQL.executeQuery(languageChechenMySQL.SELECT(connectMySQL.nameDB));
			
			while (connectMySQL.resultSetMySQL.next()){
				Question question = new QuestionChechenLanguage();
