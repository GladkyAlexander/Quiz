...
public class GetListLanguageTatarQuestionMySQLImpl implements GetListQuestionMySQL {
	@Override
	public List<Question> getListQuestion(User user, Context context) {
		...
		try {
			connectMySQL.resultSetMySQL = connectMySQL.statementMySQL.executeQuery(languageTatarMySQL.SELECT(connectMySQL.nameDB));
			
			while (connectMySQL.resultSetMySQL.next()){
				Question question = new QuestionTatarLanguage();
