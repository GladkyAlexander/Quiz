...
public class GetListRoadTransportQuestionMySQLImpl implements GetListQuestionMySQL {
	@Override
	public List<Question> getListQuestion(User user, Context context) {
		...
		try {
			connectMySQL.resultSetMySQL = connectMySQL.statementMySQL.executeQuery(roadTransportMySQL.SELECT(connectMySQL.nameDB));
			
			while (connectMySQL.resultSetMySQL.next()){
				Question question = new QuestionRoadTransport();
