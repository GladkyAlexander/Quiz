...
public class HistoryMySQLImpl implements SyntaxHistory {
    @Override
    public String CREATE(String nameDB) {
        return "CREATE TABLE if not exists`" + nameDB + "`.`history` (id int  AUTO_INCREMENT primary key NOT NULL" +
            ", question text" +
            ", right_answer text" +
            ", wrong_answer_1 text" +
            ", wrong_answer_2 text" +
    ...
    public String INSERT(String nameDB) {
        return "INSERT INTO `" + nameDB + "`.`history` (" +
            " question," +
            " right_answer," +
            " wrong_answer_1," +
            " wrong_answer_2," +
    ...
    public String SELECT(String nameDB) {
        return "SELECT * FROM `" + nameDB + "`.`history`";
    }
}
