...
public class LanguageBashkirMySQLImpl implements SyntaxLanguageBashkir {
	@Override
	public String CREATE(String nameDB) {
		return "CREATE TABLE if not exists`" + nameDB + "`.`language_bashkir` (id int AUTO_INCREMENT primary key NOT NULL" +
				       ", question text" +
				       ", right_answer text" +
				       ", wrong_answer_1 text" +
				       ", wrong_answer_2 text" +
	...
	public String SELECT(String nameDB) {
		return "SELECT * FROM `" + nameDB + "`.`language_bashkir`";
	}
}
